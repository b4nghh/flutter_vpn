...
public class CharonVpnService extends VpnService implements Runnable, VpnStateService.VpnStateListener {
	...
	private final ServiceConnection mServiceConnection = new ServiceConnection() {
		...
		public void onServiceDisconnected(ComponentName name) { /*
																 * since the service is local this is theoretically only
						...
						if (initializeCharon(mBuilderAdapter, mLogFile, mAppDir,
							...
							if (mCurrentProfile.getVpnType().has(VpnTypeFeature.USER_PASS) &&
									mCurrentProfile.getPassword() == null) { /*
																				 * this can happen if Always-on VPN is
																				 * enabled with an incomplete profile
																				 */
							...
							writer.setValue("global.rsa_pss",
									(mCurrentProfile.getFlags() & VpnProfile.FLAGS_RSA_PSS) != 0);
							writer.setValue("global.crl",
									(mCurrentProfile.getFlags() & VpnProfile.FLAGS_DISABLE_CRL) == 0);
							writer.setValue("global.ocsp",
									(mCurrentProfile.getFlags() & VpnProfile.FLAGS_DISABLE_OCSP) == 0);
	...
	/**
	 * Add a permanent notification while we are connected to avoid the service
	 * getting killed by
	...
	private Notification buildNotification(boolean publicVersion) {
		...
		if (error != ErrorState.NO_ERROR) {
			...
			if (!publicVersion && profile != null) {
				...
				// VpnProfileControlActivity.class);
				// intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
				// intent.setAction(VpnProfileControlActivity.START_PROFILE);
				// intent.putExtra(VpnProfileControlActivity.EXTRA_VPN_PROFILE_ID,
				// profile.getUUID().toString());
				// PendingIntent pending = PendingIntent.getActivity(getApplicationContext(), 0,
				// intent,
				// PendingIntent.FLAG_UPDATE_CURRENT);
				// builder.addAction(R.drawable.ic_notification_connecting,
				// getString(R.string.retry), pending);
				add_action = true;
			}
		} else {
		...
		if (!publicVersion) {
			...
			// intent,
			// PendingIntent.FLAG_UPDATE_CURRENT);
			// builder.addAction(R.drawable.ic_notification_disconnect,
			// getString(R.string.disconnect), pending);
			// }
			if (error == ErrorState.NO_ERROR) {
	...
	public class BuilderAdapter {
		...
		public synchronized void establishBlocking() {
			/*
			 * just choose some arbitrary values to block all traffic (except for what's
				...
				try {
					...
					while (true) {
						if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) { /*
																				 * just read and ignore all data,
																				 * regular
																				 * read() is not interruptible
																				 */
							...
							if (len < 0) {
								break;
									 ...
									 * this is rather ugly but on older platforms not even the NIO version of read()
									 * is interruptible
									 */
							boolean wait = true;
					...
					for (IPRange subnet : ranges.subnets()) {
						try {
							builder.addRoute(subnet.getFrom(), subnet.getPrefix());
																 ...
																 * some Android versions don't seem to like multicast
																 * addresses here,
																 * ignore it for now
																 */
							if (!subnet.getFrom().isMulticastAddress()) {
								throw e;
																					 ...
																					 * allow traffic that would
																					 * otherwise be blocked to bypass
																					 * the VPN
																					 */
